 ...
 */
export function validatePostTitle(title: string): { valid: boolean; sanitized?: string; error?: string } {
    ...
    // Check if removing HTML changed the content (means there were tags)
    if (withoutHtml !== trimmed) {
    ...
    for (const char of dangerousChars) {
        if (trimmed.includes(char)) {
            return { valid: false, error: `Title cannot contain the character: ${char}` };
        }
    }
    ...
    // Final check - title should not be empty after sanitization
